...
// 2. Generate 3 recipe variants
export const generateRecipeVariants = async (instruction: string) => {
  try {
    const prompt = `
      Depends on user instructions: ${instruction}

      Create 3 different Recipe variants.
      Each should include:
      - recipeName (with Emoji)
      - 2 line description
      - main ingredient list (without size)
    ...
    console.error("Error generating recipe variants:", error);
    throw error;
  }
};

// 3. Generate full recipe
export const generateFullRecipe = async (recipeName: string, description: string) => {
  try {
    const prompt = `
